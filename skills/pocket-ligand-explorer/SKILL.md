...
for foreground runs, the generated `03_postprocess.sh`. The postprocess script
calls `ple md-fix-pbc`, `ple pymol-metal`, and `ple md-rdc`, producing:
...
Use `--background` for long production jobs; this starts `02_run_gromacs.sh`,
writes `md_handoff/logs/md.pid`, and leaves postprocessing to be run later with
`bash md_handoff/03_postprocess.sh` after the MD finishes.
...
`md_centered_compact.gro`. Use the centered/compact pair for visualization; keep
the raw `md.xtc` for provenance.
...
Fe as an orange sphere, side chains within 4 A as sticks, carbon in PyMOL green,
and keeps N/O/S heteroatom colors blue/red/yellow with yellow distance dashes.
