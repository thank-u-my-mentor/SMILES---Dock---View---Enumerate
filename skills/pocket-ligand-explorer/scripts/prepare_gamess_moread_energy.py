#!/usr/bin/env python3
"""Build a GAMESS fixed-geometry ENERGY retry from a previous $VEC block."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create a fixed-geometry GAMESS ENERGY retry from a MOREAD vector."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Opt out of the Fe-radical default fixed fine DFT grid. Use only "
            "for reproducing old GAMESS inputs, not for new MCPB Fe-radical jobs."
        ),
    )
    ...
    lines = replace_card(
        lines,
        "$SYSTEM",
        " $SYSTEM MEMDDI=160 MWORDS=80 $END",
    )
    lines = replace_card(
        lines,
        "$CONTRL",
    ...
    if args.dft_no_coarse_grid or not args.allow_coarse_grid:
        lines = replace_card(
            lines,
            "$DFT",
            (
...
echo "[GAMESS] $JOB start $(date) NCORES=$NCORES MOREAD={args.dat} dft_grid={'legacy_coarse_allowed' if args.allow_coarse_grid and not args.dft_no_coarse_grid else 'fine_from_start'}" | tee "gamess_logs/${{JOB}}.status"
set +e
...
echo
echo "===== markers ====="
grep -E 'GUESS =|RUNTYP=|DFT CODE IS SWITCHING|NRAD0|NLEB0|ITER EX|DENSITY CONVERGED|SCF IS UNCONVERGED|SCF HAS NOT CONVERGED|FINAL U-B3LYP|S-SQUARED|TERMINATED' "gamess_logs/${{JOB}}.log" 2>/dev/null | tail -n 120 || true
echo
echo "===== recent log ====="
