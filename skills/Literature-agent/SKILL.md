...
This criteria-first mode writes `criteria_spec.md`, `task_spec.md`, annotates each paper with `criteria_status` (`criteria_pass`, `criteria_borderline`, or `criteria_fail`), and exports `homolog_candidate_seeds.csv` plus `enzyme_seed_candidates.csv` for downstream enzyme/homolog curation. It also writes `sequence_seed_candidates.csv`, `sequence_seed_tree_ready.csv`, `sequence_seed_tree_ready.xlsx`, and `sequence_seed_review.html`, which are the preferred human review surfaces for FASTA seed readiness. The sequence review adds offline evidence columns for `flavin_dependency_status`, `cofactor_class`, `catalysis_system_class`, `external_photocatalyst`, `enzyme_function_class`, `sequence_resolution_route`, expression-host evidence, PDB links, UniProt/NCBI/ENA/NCBI Gene/RefSeq queries, and organism/source evidence. Keep enzyme cofactor dependence separate from the reaction-system photosensitizer: for example, a Saccharomyces cerevisiae OYE3 paper using a Ru photosensitizer is still `confirmed_flavin_dependent`, but its `catalysis_system_class` should be `flavin_enzyme_external_photoredox`. It expands obvious synonyms for the three hard anchors, including photoenzymatic/photoenzyme/Photoenzymology/photobiocatalysis and flavin/FAD/FMN/flavoenzyme/OYE/ERED/FAP/FDH. Use `--criteria-file criteria.txt` or `.json` when the reaction sentence or goal is long.
