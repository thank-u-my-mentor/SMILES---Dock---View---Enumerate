# gromacs_smoke2 UL1-centered pocket residue scan
...
## Key outputs
- `gromacs_smoke2_ul1_pocket_frame_residue_features.csv.gz`: long table; one row per contacted residue per frame.
- `gromacs_smoke2_ul1_pocket_residue_summary.csv`: residue occupancy and median/mean distance summary.
- `gromacs_smoke2_ul1_pocket_residue_summary_by_RS.csv`: same summary split by R-like/S-like.
- `gromacs_smoke2_ul1_pocket_frame_matrix_embeddings.csv.gz`: fixed-width frame matrix with PCA/UMAP coordinates and pocket cluster labels.
